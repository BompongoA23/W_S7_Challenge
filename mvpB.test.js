...
describe('Sprint 7 Challenge Learner Tests', () => {
  ...
  Test the following. You can create separate tests or a single test with multiple assertions.
   ...
   test('throws error when one argument is not a number', () => {
   expect(() => sum(2, 'seven')).toThrow('pass valid numbers')
 })
   test('correctly adds two numbers', () => {
   expect(sum(1, 3)).toBe(4)
 })
   test('correctly adds a string number and a number', () => {
   expect(sum('1', 2)).toBe(3)
  })
   test('correctly adds two string numbers', () => {
...
test('renders a text that includes "javaScript is pretty" (case-sensitive, partial match)', () => {
  render(<HelloWorld />)
